...
/*   Updated: 2021/01/06 14:52:08 by ehakam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(cub->mlx = mlx_init()))
		ft_perror("Failed to initialize mlx!", ft_clean(cub, ERR));
	if (!(cub->window = mlx_new_window(cub->mlx, cub->cnvs.width,
	cub->cnvs.height, CUBTITLE)))
		ft_perror("Failed to initialize Window!", ft_clean(cub, ERR));
	if (!(cub->cnvs.img = mlx_new_image(cub->mlx, cub->cnvs.width,
	cub->cnvs.height)))
		ft_perror("Failed to initialize Canvas!", ft_clean(cub, ERR));
