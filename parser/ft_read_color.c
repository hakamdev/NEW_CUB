...
/*   Updated: 2021/01/04 17:35:39 by ehakam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_ceiling_color(t_cub *cub)
{
	...
	color_values = extract_value(cub->line);
	...
	if ((clr.r = ft_atoi(parts[0])) == ERR || !(clr.r >= 0 && clr.r <= 255))
		ft_perror("Invalid (Red) Value for (C)!", ft_clean(cub, ERR));
	if ((clr.g = ft_atoi(parts[1])) == ERR || !(clr.g >= 0 && clr.g <= 255))
		ft_perror("Invalid (Green) Value for (C)!", ft_clean(cub, ERR));
	if ((clr.b = ft_atoi(parts[2])) == ERR || !(clr.b >= 0 && clr.b <= 255))
